    ...
    public partial class _Defaultnew : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
           ...
           // ffhpservice ws = new ffhpservice();
            ...
            DataTable dtMergedWeight = new DataTable();
            ...
            dtB2B = dtWeight.Copy();
           // dtMergedWeight=mergedtWeightB2Bweight(dtWeight, dtB2B);
            ...
            copyDatatabletoDB(dtB2B);//to populate the B2B weight into the ffhp_purchasetemplate table
            FreeProductWeightCalc(); // to update the free products weight to the ffhp_purchasetemplate table
            weightCalculate();


        }
        public DataTable mergedtWeightB2Bweight(DataTable dtWeight,DataTable  dtB2B)//to merge regular order weight and B2B order weight
        {
            DataTable dtMerged=new DataTable();
            dtMerged.Columns.Add("ID", typeof(int));
            dtMerged.Columns.Add("name", typeof(string));
            dtMerged.Columns.Add("OrderWeight", typeof(string));
            dtMerged.Columns.Add("PurchaseWeight", typeof(string));
            dtMerged.Columns.Add("Cutdescription", typeof(string));
            dtMerged.Columns.Add("PackingStyle", typeof(string));
            dtMerged.Columns.Add("VendorName", typeof(string));

            var leftOuterJoin = from dataRows1 in dtWeight.AsEnumerable()
                         join dataRows2 in dtB2B.AsEnumerable()
                         on dataRows1.Field<int>("product_id") equals dataRows2.Field<int>("product_id")

                         select dtMerged.LoadDataRow(new object[]
             {
                dataRows1.Field<int>("product_id"),
                dataRows1.Field<string>("product_name"),
            ...
            var rightOuterJoin = from dataRows1 in dtB2B.AsEnumerable()
                                 join dataRows2 in dtWeight.AsEnumerable()
                                on dataRows1.Field<int>("product_id") equals dataRows2.Field<int>("product_id")
            ...
            using (SqlConnection conn = new SqlConnection(sqlConn))
            {
                ...
                else if (dtTemplate.Rows.Count == 0)
                {
                    ...
                    string strVendorQuery = @"select productid, D.vendorid, vendorname, packingstyle from vendor_products_default D left outer join vendordetails  V on d.vendorid=V.vendorid left outer join
                                    (Select * from testtotalweight  where convert(VARCHAR(26),Created_at,23) = CAST(CURRENT_TIMESTAMP AS DATE) and   product_Id='" + intPdtID + "') as T on D.productid=T.Product_Id where productid='" + intPdtID + "' ";
        ...
        public DataSet GetDSetwithFont()
        {
            ...
            string conn = System.Configuration.ConfigurationManager.AppSettings["MysqlConnection"].ToString();
